...
export default function IntakeForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Encode form data for Formspree or simple mailto fallback
    const mailtoBody = encodeURIComponent(
      `Name: ${form.name}\nEmail: ${form.email}\nBusiness Type: ${form.businessType}\nService: ${form.service}\nBudget: ${form.budget}\n\nMessage:\n${form.message}`
    );

    // Open email client as fallback (replace with Formspree endpoint when ready)
    window.open(
  ...
  if (submitted) {
    return (
      <section
        ...
        style={{
          background:
            "linear-gradient(160deg, #1a1208 0%, #3d2b1f 50%, #1a1208 100%)",
        }}
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {services.map((s) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 border ${
                    form.service === s
                      ? "bg-amber border-amber text-ivory"
          ...
          <div className="pt-2">
            <button
              type="submit"
              disabled={loading}
              className="w-full sm:w-auto px-10 py-4 bg-amber hover:bg-amber-light text-ivory font-semibold rounded-full transition-all duration-200 hover:scale-105 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg hover:shadow-amber/25"
              style={{ fontFamily: "var(--font-body)" }}
            >
              {loading ? (
                "Opening email..."
